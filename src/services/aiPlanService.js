...
export async function generateMealPlan(options = {}) {
  try {
    ...
    const mealPlan = await _storeMealPlan(
      weekStartDate,
      weekEndDate,
      validatedPlan,
      weeklyIntent
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a professional nutritionist. Generate meal plans as valid JSON only. Be precise with macro calculations.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      if ((error.status === 429 || error.code === 'ECONNRESET') && attempt < maxRetries) {
        ...
        console.log(
          `OpenAI API error, retrying in ${delay}ms... (attempt ${attempt}/${maxRetries})`
        );
        await _sleep(delay);
        continue;
      }
...
function _buildPrompt(macroProfile, preferences, weeklyIntent, mealsPerDay) {
  ...
  let intentGuidance = 'Normal balanced week.';
  if (weeklyIntent?.goal === 'high_satiety_low_ferritin') {
    intentGuidance =
      'Focus on high-fiber, high-protein meals that keep satiety high, while avoiding excessive iron intake.';
  } else if (weeklyIntent?.goal === 'lazy') {
    intentGuidance = 'Keep recipes very simple and quick. Few ingredients, minimal prep time.';
  } else if (weeklyIntent?.goal === 'gourmet') {
    intentGuidance =
      'Create more elaborate, restaurant-style meals, but still compatible with macros.';
  }
  ...
  return `Generate a complete 7-day meal plan with ${mealsPerDay} meals per day.
...
MEAL DISTRIBUTION (order in the day):
${mealTypesText}

DIETARY REQUIREMENTS:
- EXCLUDED ingredients (MUST NOT use): ${exclusions}
- Preferred cuisines: ${cuisinePreferences}

WEEKLY INTENT / CONTEXT:
${intentGuidance}

IMPORTANT RULES:
1. Each meal MUST include exact ingredient quantities (grams, ml, etc.).
...
5. Provide clear, step-by-step cooking instructions for each recipe.
6. Vary recipes across the week (do not repeat the same recipe more than twice).
...
function _validateExclusions(plan, preferences) {
  ...
  for (const day of plan.days) {
    for (const meal of day.meals) {
      const ingredientsText = meal.recipe.ingredients.join(' ').toLowerCase();

      for (const excludedItem of excluded) {
        if (ingredientsText.includes(excludedItem)) {
          throw new Error(
            `Excluded ingredient "${excludedItem}" found in recipe "${meal.recipe.title}". Please regenerate the meal plan.`
          );
        }
      }
    }
